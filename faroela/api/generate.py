...
parser.add_argument('-t', '--language', default='c++', help='The language to generate interfaces for')
...
def generate_cxx(f, api):
	...
	for id, values in api['functions'].items():
		f.write(f'\t\tFAROELA_COMMON_EXPORT link_bool faroela_{id}(')
...
def splice(f, api, position, language):
	try:
		path = api['extra'][position][language]
	except:
		path = None

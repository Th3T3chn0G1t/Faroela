...
extern "C" {
	namespace faroela::api {
		...
		FAROELA_COMMON_EXPORT link_bool faroela_initialize(faroela::context**, int, char**);
		FAROELA_COMMON_EXPORT void faroela_shutdown(faroela::context**);

		FAROELA_COMMON_EXPORT void faroela_log(faroela::context*, verbosity, const char*);

		// Signal a HID as having been connected or disconnected. Disconnection must refer to a
		// previously-connected port.
		FAROELA_COMMON_EXPORT link_bool faroela_hid_status(faroela::context*, hid::port, link_bool);
		FAROELA_COMMON_EXPORT link_bool faroela_hid_button_event(faroela::context*, hid::port, hid::button, link_bool);
		FAROELA_COMMON_EXPORT link_bool faroela_hid_axis_event(faroela::context*, hid::port, hid::axis, float);
	}
}
